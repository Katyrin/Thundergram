...
suspend fun TelegramFlow.checkAuthenticationPassword(password: String?): Unit =
    sendFunctionLaunch(CheckAuthenticationPassword(password))
...
suspend fun TelegramFlow.createTemporaryPassword(
    password: String?,
    validFor: Int
): TemporaryPasswordState = sendFunctionAsync(CreateTemporaryPassword(password, validFor))
 ...
 */
suspend fun TelegramFlow.getPasswordState(): PasswordState = sendFunctionAsync(GetPasswordState())
...
suspend fun TelegramFlow.getTemporaryPasswordState(): TemporaryPasswordState =
    sendFunctionAsync(GetTemporaryPasswordState())
...
suspend fun TelegramFlow.recoverAuthenticationPassword(recoveryCode: String?): Unit =
    sendFunctionLaunch(RecoverAuthenticationPassword(recoveryCode))
...
suspend fun TelegramFlow.recoverPassword(recoveryCode: String?): PasswordState =
    sendFunctionAsync(RecoverPassword(recoveryCode))
...
suspend fun TelegramFlow.requestAuthenticationPasswordRecovery(): Unit =
    sendFunctionLaunch(RequestAuthenticationPasswordRecovery())
...
suspend fun TelegramFlow.requestPasswordRecovery(): EmailAddressAuthenticationCodeInfo =
    sendFunctionAsync(RequestPasswordRecovery())
...
): PasswordState = sendFunctionAsync(
    SetPassword(oldPassword, newPassword, newHint, setRecoveryEmailAddress, newRecoveryEmailAddress)
)