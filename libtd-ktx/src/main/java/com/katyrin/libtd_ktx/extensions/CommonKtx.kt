...
interface CommonKtx : BaseKtx {
     ...
     */
    suspend fun Background.remove(): Unit = api.removeBackground(id)
     ...
     *
     */
    suspend fun Notification.remove(notificationGroupId: Int): Unit =
        api.removeNotification(notificationGroupId, id)

    /**
     * Suspend function, which deletes a profile photo. If something changes, updateUser will be sent.
     */
    suspend fun ProfilePhoto.delete(): Unit = api.deleteProfilePhoto(id)
     ...
     */
    suspend fun RemoteFile.get(fileType: FileType?): File = api.getRemoteFile(id, fileType)

    /**
     * Suspend function, which terminates a session of the current user.
     */
    suspend fun Session.terminate(): Unit = api.terminateSession(id)
    ...
    ): PaymentResult = api.sendPaymentForm(chatId, messageId, orderInfoId, id, credentials)
     ...
     */
    suspend fun StickerSet.setSupergroup(supergroupId: Int): Unit =
        api.setSupergroupStickerSet(supergroupId, id)
}