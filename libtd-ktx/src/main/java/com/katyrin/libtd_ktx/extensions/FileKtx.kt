...
interface FileKtx : BaseKtx {
     ...
     */
    suspend fun File.cancelDownload(onlyIfPending: Boolean): Unit =
        api.cancelDownloadFile(id, onlyIfPending)
     ...
     */
    suspend fun File.cancelUpload(): Unit = api.cancelUploadFile(id)

    /**
     * Suspend function, which deletes a file from the TDLib file cache.
     */
    suspend fun File.delete(): Unit = api.deleteFile(id)
    ...
    suspend fun File.download(
        ...
        limit: Int,
        synchronous: Boolean
    ): File = api.downloadFile(id, priority, offset, limit, synchronous)
     ...
     */
    suspend fun File.getAttachedStickerSets(): TdApi.StickerSets = api.getAttachedStickerSets(id)
     ...
     * @return [TdApi.File] Represents a file.
     */
    suspend fun File.get(): File = api.getFile(id)
     ...
     */
    suspend fun File.getDownloadedPrefixSize(offset: Int): TdApi.Count =
        api.getFileDownloadedPrefixSize(id, offset)
     ...
     */
    suspend fun File.readPart(offset: Int, count: Int): TdApi.FilePart =
        api.readFilePart(id, offset, count)
}