...
suspend fun TelegramFlow.addProxy(
    ...
    enable: Boolean,
    type: ProxyType?
): Proxy = sendFunctionAsync(AddProxy(server, port, enable, type))

/**
 * Suspend function, which disables the currently enabled proxy. Can be called before authorization.
 */
suspend fun TelegramFlow.disableProxy(): Unit = sendFunctionLaunch(DisableProxy())
...
suspend fun TelegramFlow.editProxy(
    ...
    enable: Boolean,
    type: ProxyType?
): Proxy = sendFunctionAsync(EditProxy(proxyId, server, port, enable, type))
 ...
 * @param proxyId Proxy identifier.
 */
suspend fun TelegramFlow.enableProxy(proxyId: Int): Unit = sendFunctionLaunch(EnableProxy(proxyId))
 ...
 */
suspend fun TelegramFlow.getProxyLink(proxyId: Int): Text = sendFunctionAsync(GetProxyLink(proxyId))
 ...
 */
suspend fun TelegramFlow.pingProxy(proxyId: Int): Seconds = sendFunctionAsync(PingProxy(proxyId))
 ...
 * @param proxyId Proxy identifier.
 */
suspend fun TelegramFlow.removeProxy(proxyId: Int): Unit = sendFunctionLaunch(RemoveProxy(proxyId))
...
suspend fun TelegramFlow.testProxy(
    ...
    dcId: Int,
    timeout: Double
): Unit = sendFunctionLaunch(TestProxy(server, port, type, dcId, timeout))