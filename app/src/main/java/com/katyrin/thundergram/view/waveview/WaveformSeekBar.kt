...
import com.katyrin.thundergram.view.waveview.utils.WaveGravity
import com.katyrin.thundergram.view.waveview.utils.dp
import java.io.*
...
open class WaveformSeekBar @JvmOverloads constructor(
    ...
    private var mMaxValue = context.dp(2).toInt()
    ...
    var waveGap: Float = context.dp(2)
        set(value) {
            field = value
            invalidate()
        }
    ...
    var waveWidth: Float = context.dp(7)
        set(value) {
            field = value
            invalidate()
        }
    ...
    var waveCornerRadius: Float = context.dp(5)
        set(value) {
            field = value
            invalidate()
        }
    ...
    var markerWidth: Float = context.dp(1)
        set(value) {
            field = value
            invalidate()
        }
    ...
    var markerTextSize: Float = context.dp(12)
        set(value) {
            field = value
            invalidate()
        }

    var markerTextPadding: Float = context.dp(2)
        set(value) {
            field = value
            invalidate()
        }
    ...
    private fun setMaxValue() {
        mMaxValue = sample?.maxOrNull()?.toInt() ?: 0
    }

    fun setSampleFrom(audio: Uri) {
        sample = fileToBytes(audio)
    }

    private fun fileToBytes(uri: Uri): ByteArray {
        ...
        return byteArrayOf(83, 56, 30, 74, 62, 30, 30, 17, 74, 62, 74, 56, 30, 30, 74, 62, 30, 30)
    }
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        if (visibleProgress > 0) when (event?.action) {
            ...
            MotionEvent.ACTION_DOWN ->
                if (isParentScrolling()) mTouchDownX = event.x
                else updateProgress(event)
            MotionEvent.ACTION_MOVE -> updateProgress(event)
            MotionEvent.ACTION_UP -> {
                if (abs(event.x - mTouchDownX) > mScaledTouchSlop) updateProgress(event)
                performClick()
            }
        }
        return true
    ...
    private fun getProgress(event: MotionEvent): Float =
        if (visibleProgress > 0)
            (mProgress - visibleProgress * (event.x - mTouchDownX) / getAvailableWidth())
                .coerceIn(0F, maxProgress)
        else maxProgress * event.x / getAvailableWidth()
