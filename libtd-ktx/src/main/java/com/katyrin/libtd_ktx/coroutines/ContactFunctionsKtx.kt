...
suspend fun TelegramFlow.addContact(contact: Contact?, sharePhoneNumber: Boolean): Unit =
    sendFunctionLaunch(TdApi.AddContact(contact, sharePhoneNumber))
...
suspend fun TelegramFlow.changeImportedContacts(contacts: Array<Contact>?): ImportedContacts =
    sendFunctionAsync(TdApi.ChangeImportedContacts(contacts))

/**
 * Suspend function, which clears all imported contacts, contact list remains unchanged.
 */
suspend fun TelegramFlow.clearImportedContacts(): Unit =
    sendFunctionLaunch(TdApi.ClearImportedContacts())
 ...
 */
suspend fun TelegramFlow.getContacts(): Users = sendFunctionAsync(TdApi.GetContacts())
...
suspend fun TelegramFlow.importContacts(contacts: Array<Contact>?): ImportedContacts =
    sendFunctionAsync(TdApi.ImportContacts(contacts))
...
suspend fun TelegramFlow.removeContacts(userIds: IntArray?): Unit =
    sendFunctionLaunch(TdApi.RemoveContacts(userIds))
...
suspend fun TelegramFlow.searchContacts(query: String?, limit: Int): Users =
    sendFunctionAsync(TdApi.SearchContacts(query, limit))